...
## Runs warmup first so a fresh sandbox pays the numba JIT compile before the
## timed suite (not inside it). See docs/TESTING.md and the `warmup` target.
test: warmup
	$(VENV_PY) -m pytest -n auto -m "not slow" -q
...
compileall:
	$(VENV_PY) -m compileall -q models/ tests/
...
import numpy as np; \
y0 = np.array([100.0, 1e-4, 500.0, 250.0, 0.0]); \
x_eval = np.linspace(1e-4, 0.0, 20); \
fn(y0, 2.0, -1.0, 1e-4, 0.0, x_eval, \
