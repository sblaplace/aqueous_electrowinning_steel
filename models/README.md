...
| `deposit_aging.py` | **Deposit self-annealing — the time-stamped σ/HV/ρ (V6 §5.2):** log-time recovery σ(t)/σ0 = 1−A ln(1+t/τ_eff) (HV/Rho scaled, stress-H coupled), Arrhenius T shift + H-accelerated recovery, live `hydrogen_trapping` D_eff slab H-egress for foil thickness. Defines the 24 h (18–30 h @ 20±2 °C) metrology standard and a fail-soft `aging_hours` QA hook for `run_record` so 4 h vs 48 h runs are comparable — \"does saccharin move the number, or does the clock?\" |
| `strain_aging.py` | **Strain aging — N pins the press (V6 §7.1):** ammoniacal [N] uptake via live `ammonium_buffer` free-NH₃, Cottrell–Bilby t★(T,C_N) with Wert–Zener D_N(T), Δσ_sat(C_N) yield-point return (2/3 law) and grain-scaled Lüders strain. Verdicts `clears / conditional (1–2 % skin-pass) / fails` and makes \"ammonium vs boric buffer\" a composition, not just regulatory, choice |
| `product_oxidation.py` | **Product oxidation, drying & pyrophoricity — the post-harvest oxygen budget (V6 §1.2):** specific-surface bookkeeping from `cell_architecture` product form (powder spheres × dendritic roughness, web two-face), Mott–Cabrera log film ↔ parabolic hot-branch crossover, Semenov thermal-runaway T_crit (Arrhenius generation vs bulk loss) with an RK4 ignition-delay transient, and the passivation protocol that becomes the product spec (≤1.5 wt% O, non-pyrophoric). Feeds `melt_balance` O pickup live — the safety gate for "dried hot and fast" |
| `rinse_carryover.py` | **Rinse carryover — bath liquor becomes melt-shop sulfur (V6 §1.3):** Landau–Levich withdrawal film on webs (viscosity/shear fixed-point against `bath_rheology`) + powder-cake liquor, exact counter-current cascade (`F_n = 1/Σ r^k`), charge-borne S/Na/B budgets per tonne, water demand, tank-return loop, final-rinse conductivity endpoint. Feeds `melt_balance` charge S live — "how much rinsing?" computed as a steel-grade decision: unrinsed foil carries ~0.4 wt% S; three counterflow stages at r=5 puts a powder at 15 ppm |
...
| `briquetting.py` | **Briquetting / densification — the product-form gate (V6 §1.4):** Heckel compaction with the V6 ranking relation 1/K = 3σ_y (deposit σ_y live from `mechanical_properties`), exact P↔D inversions pinned to the tap-state A branch, green-strength master curve, porous-modulus springback, die-wall ejection, press specific work → delivered kWh/t, handling-fines ratio form, Jenike rathole vs bridging flow screens (ferromagnetic agglomeration term), and the shippable-product spec block (density / crush / fines / residual O) for `feedstock_logistics.py`/`dark_mill.py`. Feeds `melt_balance` the shipped-fines fraction live — cold pressing at the design force is conditional on sink-and-size, HBI-style hot pressing at 600 °C clears |
...
| `ladder_tea.py` | **Ladder × TEA — full plant costing for every rung.** Runs each rung through the complete cost stack the ladder deliberately excludes (feedstock, electrolyte make-up, anode wear, overhead, maintenance, insurance, labour, ore-side plant) at a common nameplate from the *same live* architecture/energy state; reports full-TEA margin, NPV/payback/IRR, the itemised **screening gap**, and the pairwise rank-flip count between the contribution screen and the full TEA (currently **0 flips** — the ranking is cost-structure-robust). Accepts a `dark_mill` site for grid-price/labour overrides; regenerates `docs/LADDER_TEA.md` |
| `ti_hydriding.py` | **Ti drum hydriding & campaign life (V6 §4.1):** HER partial current at the *live* drum operating point → absorbed H flux (k_entry × shielded area), α-Ti diffusion × terminal solubility → δ-hydride case growth, spall criterion, and the G_c drift multiplier that makes "peel passes day one, fails in month two" concrete (composites with `substrate_passivation`). Verdicts spoken across the honest k_entry × f_shield band with a **design target** (`≤ 3e-5` at live j_op) for the re-passivation/pinhole-audit programme; campaign-ledger row for `closed_loop.py` |
...
python -m models.deposit_aging              # RT self-annealing σ/HV/ρ drift, H-coupled recovery time, metrology standard (V6 §5.2)
python -m models.strain_aging               # Ammoniacal N → Cottrell t★(T,C_N) → Δσ_y & Lüders, skin-pass prescription (V6 §7.1)
